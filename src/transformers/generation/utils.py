...
class GenerationMixin:
    ...
    def _get_logits_warper(
        ...
        if generation_config.top_a is not None and generation_config.top_a > 0.0:
            warpers.append(TopALogitsWarper(top_a=generation_config.top_a, min_tokens_to_keep=min_tokens_to_keep))
        if generation_config.tfs is not None and generation_config.tfs < 1.0:
            warpers.append(TailFreeSamplingLogitsWarper(threshold=generation_config.tfs))
    ...
    def _get_logits_processor(
        ...
        if (generation_config.repetition_penalty is not None and generation_config.repetition_penalty > 1.0) or (generation_config.repetition_penalty_frequency is not None and generation_config.repetition_penalty_frequency > 0.0) or (generation_config.repetition_penalty_presence is not None and generation_config.repetition_penalty_presence > 0.0):
            processors.append(RepetitionPenaltyLogitsProcessor(penalty=generation_config.repetition_penalty, m=generation_config.repetition_penalty_slope, penalize_last=generation_config.repetition_penalty_range, alpha_frequency=generation_config.repetition_penalty_frequency, alpha_presence=generation_config.repetition_penalty_presence, whitelist=generation_config.repetition_penalty_whitelist))
